...
| Tests | Karma/Jasmine installed, `skipTests: true` everywhere — **zero spec files** |
...
- `src/app/core.module.ts` **and** `src/app/core/core.module.ts` — two different `CoreModule` classes; the app imports the empty one, so `core/core.module.ts` is dead code
...
- `features/package/` is a full module whose template is literally `<p>package works!</p>`. Its route is unreachable — `app-routing` redirects `package/:id` into the catalog module, and `PackageModule` is never lazy-loaded.
- `features/user/` template is `<p>user works!</p>` — Angular starter placeholder, reachable at `/user`. **Violates "no Angular starter content".**
...
- `assets/logo-ps5.png` is referenced by the header and **does not exist** — broken image on every page.
...
- **`CartService` mutates state in place** (`existingItem.quantity += …`) before emitting, so the `BehaviorSubject` re-emits the same array reference — `OnPush` change detection would not fire.
...
- No `HttpInterceptor`, no auth plumbing, no global `ErrorHandler` — nothing to hang a real backend on.
...
- `platform` is a free-form `string` compared against `'PlayStation'` literals harvested from the data at runtime — no typed platform domain.
...
### 3.6 Fulfillment — entirely absent
Delivery promises live in free-text Hebrew inside `description` ("אספקה מיידית"). There is no fulfillment method, no fulfillment status, and no delivery record — therefore no honest way to distinguish "instant code" from "manual service, 5–30 minutes".
...
6. `Math.random() < 0.8` payment simulation lives behind the same `PaymentService` name a real provider would use — a fake integration with no "mock" marker.
...
- The "טען עוד חבילות" (load more) button has **no click handler** — a dead button.
- The quick-view (eye) button and the buy button both call `onBuyNow` — two affordances, one behaviour.
- No skeletons; a spinner replaces the whole page on every load.
- No route for FAQ, support, reviews, terms, privacy, refunds, or accessibility — all effectively mandatory for an Israeli consumer store.
- `anyComponentStyle` budget is 4 kB; `catalog.component.scss` (614 lines) will breach it as it grows.

### 3.10 Configuration
`environment.ts` and `environment.development.ts` are both `export const environment = {}` — no `apiBaseUrl`, no feature flags. There is no `environment.production.ts`; production reuses the base file.
